...
class IconCounter:
    def __init__(self, icon_path, image_source, max_dimension=8000):
    ...
    def count_icons_robust(self, angles=None, threshold=0.6, show_matches=True,
                            use_color=False, n_workers=None, image_to_process=None, log_fn=None,
        ...
        img_match  = target if use_color else cv2.cvtColor(target, cv2.COLOR_BGR2GRAY)
        rotated    = {}
        for a in angles:
            ri = self.rotate_image(self.icon, a)
            rotated[a] = ri if use_color else cv2.cvtColor(ri, cv2.COLOR_BGR2GRAY)
        
        # Also create mirrored versions if requested (Horizontal, Vertical, and Both)
        if include_mirrored:
            log(f"  Including mirrored symbols (H/V flips + rotations)")
            # 1=Horizontal, 0=Vertical, -1=Both
            for code, suffix in [(1, 'fH'), (0, 'fV'), (-1, 'fB')]:
                flipped_template = cv2.flip(self.icon, code)
                for a in angles:
                    ri = self.rotate_image(flipped_template, a)
                    rotated[f'{a}_{suffix}'] = ri if use_color else cv2.cvtColor(ri, cv2.COLOR_BGR2GRAY)
        
        tasks = [(a, rotated[a], img_match, threshold) for a in rotated.keys()]
        ...
        for m in filtered:
            m['x']=int(m['x']/self.scale_factor); m['y']=int(m['y']/self.scale_factor)
            m['w']=int(m['w']/self.scale_factor); m['h']=int(m['h']/self.scale_factor)
...
def count_symbol(symbol_path, floor_plan_path,
    ...
    # Use HSV filter for colored symbols (like yellow), RGB for others
    if sym_analysis['colored_ratio'] > 0.05:
        log("  Using HSV color filter (symbol has distinctive color)")
        filtered_cv = filter_by_hsv_color(floor_cv, symbol_bgr, tolerance_h=15, tolerance_sv=50)
    else:
        log("  Using RGB color filter (symbol appears grey/black)")
    ...
    log("  Scan 1: color-filtered image (with mirrored symbols)")
    _, _, matches_filt = counter.count_icons_robust(
        angles = list(range(0, 360, 30)), threshold=0.5,
        show_matches=False, use_color=False,
        n_workers=multiprocessing.cpu_count(), include_mirrored=True, log_fn=log_fn)

    log("  Scan 2: original image (with mirrored symbols)")
    orig_cv     = cv2.imread(floor_plan_path)
    scaled_orig = cv2.resize(orig_cv, None, fx=counter.scale_factor,
                              fy=counter.scale_factor, interpolation=cv2.INTER_AREA)
    _, _, matches_orig = counter.count_icons_robust(
        angles = list(range(0, 360, 30)), threshold=0.5,
        show_matches=False, use_color=False,
        n_workers=multiprocessing.cpu_count(), image_to_process=scaled_orig,
        include_mirrored=True, log_fn=log_fn)
