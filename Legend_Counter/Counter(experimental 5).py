...
def _match_akaze(icon_bgr, image_bgr, threshold=0.6):
    """
    Finds matches of icon_bgr within image_bgr using a sliding window AKAZE approach.
    Returns list of match dictionaries:
      [{'x': int, 'y': int, 'w': int, 'h': int, 'angle': 0, 'confidence': float}]
    """
    matches = []
    ...
    # Detect and compute keypoints for the icon once
    kp1, des1 = akaze.detectAndCompute(icon_bgr, None)
    ...
    for y in range(0, h, stride_y):
        for x in range(0, w, stride_x):
            # Extract patch
            patch = image_bgr[y:min(y+win_h, h), x:min(x+win_w, w)]
            
            # If patch is smaller than the target symbol, skip
            if patch.shape[0] < ih or patch.shape[1] < iw:
                continue
                
            kp2, des2 = akaze.detectAndCompute(patch, None)
            ...
            if M is not None:
                mask_list = mask.ravel().tolist() if mask is not None else []
                inliers = [m for i, m in enumerate(good_matches) if mask_list[i]]
                
                if len(inliers) >= 4:
                    pts = np.float32([ [0,0], [0,ih-1], [iw-1,ih-1], [iw-1,0] ]).reshape(-1, 1, 2)
                    try:
                        dst = cv2.perspectiveTransform(pts, M)
                        px, py, bw, bh = cv2.boundingRect(np.int32(dst))
                        
                        if bw > 0 and bh > 0:
                            confidence = len(inliers) / len(kp1)
...
class IconCounter:
    def __init__(self, icon_path, image_source, max_dimension=8000000):
        """
        Initialize IconCounter.
    ...
    def _precompute_valid_tiles(self, image, is_tiled, tile_size=2000, overlap=200):
    ...
    def count_icons_robust(self, threshold=0.6, show_matches=True,
                            image_to_process=None, log_fn=None):
        """
        Count icons using AKAZE invariant feature matching.
        """
        def log(msg):
        ...
        log(f"\n  🔍 Scanning using AKAZE invariant matching...")
        
        # Perform AKAZE match
        all_matches = _match_akaze(self.icon, target, threshold)
        
        log(f"\n  📊 Total raw hits: {len(all_matches)}")
...
def count_symbol(symbol_path, floor_plan_path,
    ...
    counter = IconCounter(highlighted_symbol, highlighted_floor, max_dimension=8000000)

    log("  Scan: highlighted image (using AKAZE)")
